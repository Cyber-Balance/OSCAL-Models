using AutoBogus;
using Bogus;
using Bogus.Extensions;
using System.Linq;

namespace Nist.Oscal.Tests.Bogus
{
    class ObjectivesAndMethodsFaker : AutoFaker<Models.Core.ObjectivesAndMethods>
    {
        public ObjectivesAndMethodsFaker()
        {

            RuleFor(i => i.ControlId, f => f.Lorem.Word());
            RuleFor(i => i.Description, f => f.Lorem.Paragraph());
            RuleFor(r => r.Props, f => new PropFaker().GenerateBetween(1, 3).OrEmptyList(f, 0.5f));
            RuleFor(r => r.Links, f => new LinkFaker().GenerateBetween(1, 3).OrEmptyList(f, 0.5f));
            RuleFor(r => r.Parts, f => new PartFaker().GenerateBetween(1, 3));
            RuleFor(r => r.Remarks, f => f.Lorem.Paragraph().OrDefault(f, 0.5f));
        }
    }
}
